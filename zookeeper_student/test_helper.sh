...
$THRIFT_CC --gen java:generated_annotations=suppress db.thrift 2>&1 | tee -a $LOGS
...
cp ../Client.java .
$JAVA_CC -cp .:gen-java/:"lib/*" Client.java 2>&1 | tee -a $LOGS
...
for CASE in `seq 0 4`; do
	...
	JAVA_HOME=$JAVA_HOME $ZKPATH/bin/zkCli.sh -server $ZKSTRING rmr $ZKNODE > /dev/null
	JAVA_HOME=$JAVA_HOME $ZKPATH/bin/zkCli.sh -server $ZKSTRING rmr $ZKNODE_BARRIER > /dev/null
	sleep 1
	JAVA_HOME=$JAVA_HOME $ZKPATH/bin/zkCli.sh -server $ZKSTRING create $ZKNODE "" > /dev/null
	...
	taskset -c 0-7 $JAVA_JVM -cp .:gen-java:"lib/*" GraderBarrier $ZKSTRING $ZKNODE_BARRIER 3 &> /dev/null

	echo Starting client at `date`  | tee -a $LOGS
	rm -f grader_execution.log
	rm -f grader_scores.log
	rm -f $OUTPUT
	CMD="Client $ZKSTRING $ZKNODE ${THREADS[$CASE]} ${DURATION[$CASE]} ${KEYSPACE_SIZE[$CASE]}"
	echo Command: $CMD
